...
def plot_heatmap(df, row_feats, col_feats, title):
    corr = df.corr().loc[row_feats, col_feats]
    plt.figure(figsize=(len(col_feats), len(row_feats)))
...
def run_clip_level_plots(df):
    ...
    plot_heatmap(df, fv, hv, 'Acoustic vs Hume correlations')
    plot_heatmap(df, fv, pv, 'Acoustic vs Praat correlations')
...
def compute_and_plot_composite(df):
    ...
    dfz['Int_z']   = (dfz['AvgIntensity_dB'] - dfz['AvgIntensity_dB'].mean()) / dfz['AvgIntensity_dB'].std()
    res = []
    for emo in LABELS:
        sub = dfz.dropna(subset=[f'hume_{emo}','Pitch_z','Int_z'])
        r1,_ = pearsonr(sub['Pitch_z'], sub[f'hume_{emo}'])
        r2,_ = pearsonr(sub['Int_z'],   sub[f'hume_{emo}'])
        res.append({'emo': emo, 'f_r': r1, 'a_r': r2})
    cdf = pd.DataFrame(res).set_index('emo')
    cdf[['f_r','a_r']].plot.bar(); plt.show()
...
def compare_ekberg(imps):
    ...
    """
    # map FEATURE_STATS keys to your acoustic feature names in 'imps'
    feat_map = {
        'pitch':            'mean_pitch_hz',
        ...
        # if you have additional mappings, add them here
    }
    ek = {}
    ...
    plt.scatter(comp['max_z'], comp['imp'])
    for feat in comp.index:
        plt.text(comp.loc[feat,'max_z'], comp.loc[feat,'imp'], feat)
    plt.xlabel('Ekberg max |z|')
    plt.ylabel('Model feature importance')
    plt.title(f'ρ = {rho:.2f}, p = {p:.3f}')
    plt.tight_layout(); plt.show()
